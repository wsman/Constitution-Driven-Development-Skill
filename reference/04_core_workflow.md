...
2. **自动化熵值优化**: 运行 `python scripts/measure_entropy.py --optimize --dry-run` 分析热点和生成优化计划
3. **审查并执行优化**: 根据优化计划执行熵值降低操作
4. **手动重构**: 如果自动化优化不足，执行 `WF-206_refactor_protocol.md` (如可用) 或手动重构
5. **优先级调整**: 技术债务修复优先于业务功能
6. **监控恢复**: 持续测量熵值，直到 $H_{sys} \le 0.5$
