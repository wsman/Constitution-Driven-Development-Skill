...
| 脚本名称 | 参数支持 | 使用场景 |
|----------|----------|----------|
| `cdd-feature.py` (v2.1+) | `--target` (默认: 当前目录) | 在目标项目中生成特性文档，支持跨项目运行 |
| `measure_entropy.py` | `--project` (默认: 当前目录) | 测量目标项目的系统熵值 |
| `verify_versions.py` | `--project` (默认: 当前目录) | 验证目标项目的版本一致性 |
| `deploy_cdd.py` | `--target` (默认: 当前目录) | 从CDD SKILL复制模板到目标项目 |
