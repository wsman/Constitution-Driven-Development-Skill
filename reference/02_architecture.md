...
|------|------|----------|----------|----------|
| `deploy_cdd.py` | v2.0 | §102.3 同步公理 | 孢子部署，初始化Memory Bank | **开始新项目** (State A之前) |
| `cdd-feature.py` | v2.1.0 | §141 状态机公理 | 特性脚手架，生成T2 Specs | **状态A→B转换** |
| `cdd_audit.py` | v1.0 | §201.3 三阶验证公理 | 宪法审计，运行Gate 1-3 | **状态C→D转换** |
| `measure_entropy.py` | v1.4.0 | §201.5 熵减公理 | 熵值计量，计算$H_{sys}$ | **监控系统健康** |
| `verify_versions.py` | v1.7.0 | §102.3 同步公理 | 版本同步，检查和修复一致性 | **修复版本漂移** |
