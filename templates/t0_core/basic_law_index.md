...
**版本**: v1.7.0 (基本法版本独立于项目版本)
**状态**: [🟢 活跃 | 🟡 维护中 | 🔴 重构中]
**项目**: [项目名称]
**最后更新**: [YYYY-MM-DD]
**说明**: [项目名称]的基本法核心公理摘要，作为 CDD 开发时始终加载的内核。本文件定义了项目的宪法约束和核心原则。

*注：基本法版本 v1.7.0 是宪法模板版本，与项目业务版本独立。宪法变更需遵循 WF-amend 修正案协议。*
## 法律层级体系
...
### 🏛️ 基本法 (宪法核心) §1-§299
**性质**：核心公理、基本原则  
**示例**：§102.3（宪法同步）、§152（单一真理源）、§300.1（孢子协议）
**约束力**：最高，所有其他法律必须遵守基本法

### ⚙️ 技术法 §300-§499  
**性质**：数学公理、技术标准、实现规范  
**示例**：§300.1（核心数学公理）、§301-§309（输入输出）、§438（工具调用）
**约束力**：中等，在基本法框架内规定技术实现

### 🔄 程序法 §500-§599
**性质**：工作流协议、操作流程、协同规范  
**示例**：§500-§599（工作流协议）、WF-xxx（工作流实施）
**约束力**：具体，规定开发过程的步骤和协作

### ⚖️ 法律冲突解决原则
1. **宪法至上**：基本法优先于技术法和程序法
2. **新法优于旧法**：同一层级中，后制定的法律优先
3. **特别法优于一般法**：针对特定领域的条款优先于一般条款
4. **法律保留**：只有基本法可限制公民（开发者）基本权利

### 📝 法律引用规范
- **基本法**：引用为“宪法§xxx”（如宪法§300.1）
- **技术法**：引用为“技术法§xxx”（如技术法§300.1）
- **程序法**：引用为“程序法§xxx”（如程序法§501）
- **标准**：引用为“DS-xxx”（如DS-050）
- **工作流**：引用为“WF-xxx”（如WF-201）

**注意**：§300.1条款同时存在于基本法（孢子协议）和技术法（核心数学公理），是 **法律分层** 的典型示例。代码引用应明确层级。
...
| **§300.1** | 孢子协议 | 防止工具链对自己进行操作（递归污染），实现智能隔离 |
