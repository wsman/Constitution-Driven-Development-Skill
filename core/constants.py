...
#
# 2. 成分视角（系统内部分析） - 当前为理论模型
#   H_sys_component = 0.4 * H_cog + 0.3 * H_struct + 0.3 * H_align
#
# 成分熵值指标:
#   H_cog    = 认知负载 - 开发者理解系统所需的认知工作量
#   H_struct = 结构离散 - 文件间连接缺失程度
#   H_align  = 同构偏离 - 代码实现与架构约束的偏差
#
# 两种视角的关系:
#   当系统完全符合宪法时，高合规分数（低H_sys）通常对应低成分熵值。
#   两者结合使用可获得系统健康度的完整视图。
#
...
#
# 详细说明参见 reference.md 和 SKILL.md 中的"双视角熵值模型"章节。
...
THRESHOLD_DANGER: Final[float] = 0.7     # 危险阈值
